...
def build_bench_table(active_rows: list[dict], assignment: dict[str, dict | None]) -> list[dict]:
    ...
    if st.button(
        ...
        try:
            ...
            with st.spinner("Refreshing all data..."):
                proc = subprocess.run(
                    ["/bin/bash", "/app/runtime/refresh_all.sh"],
                    capture_output=True,
                    text=True,
                )
            ...
            if proc.returncode == 0:
                try:
                    ...
                    pass
                st.success("Refresh All completed.")
                st.rerun()
            else:
                st.error("Refresh All failed.")
        finally:
...
bench_rows = build_bench_table(active_rows, assignment)
...
with tab_fa:
    ...
    if not available_batters:
        ...
        fa_rows = []
        for r in available_batters:
            fa_rows.append(
                {
                    "Player": r.get("player_display", ""),
                    "Rank": r.get("ranking", ""),
                    "Delta": r.get("comparison_delta", ""),
                    "Game": game_with_pitcher(r),
                    "Status": r.get("status_display", ""),
                    "Rank Reason": compress_rank_reason(r.get("note_short", "")),
                }
